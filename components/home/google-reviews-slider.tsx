      ...
      <div key={index} className="review-slide-in grid gap-4 sm:grid-cols-2">
        {visible.map((review, cardIndex) => (
          <a key={`${review.name}-${cardIndex}`} href={review.url} target="_blank" rel="noopener noreferrer" className={`${cardIndex === 1 ? "hidden sm:block" : "block"} rounded-2xl border border-[#dadce0] bg-white p-4 shadow-[0_3px_10px_rgba(60,64,67,.12)] transition hover:-translate-y-0.5 hover:shadow-[0_6px_18px_rgba(60,64,67,.16)]`}>
            <div className="flex items-start justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-[#e8f0fe] text-sm font-bold uppercase text-[#1a73e8]">{review.name.charAt(0)}</div><div className="min-w-0"><div className="truncate text-xs font-bold text-[#202124]">{review.name}</div><div className="mt-0.5 text-[10px] text-[#70757a]">{review.time}</div></div></div><FcGoogle className="h-5 w-5 shrink-0" aria-hidden="true" /></div>
