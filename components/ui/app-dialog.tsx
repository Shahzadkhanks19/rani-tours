...
export function AppDialog({ open, title, message, tone = "info", confirmLabel = "OK", cancelLabel = "Cancel", onConfirm, onCancel }: Props) {
  ...
  const handleBackdropMouseDown = (event: MouseEvent<HTMLDivElement>) => {
    if (event.currentTarget === event.target && onCancel) onCancel();
  };

  return (
    <div className="fixed inset-0 z-[9999] grid place-items-center bg-[#08130d]/55 px-4 backdrop-blur-[2px]" role="presentation" onMouseDown={handleBackdropMouseDown}>
      <section role="dialog" aria-modal="true" aria-labelledby="app-dialog-title" aria-describedby="app-dialog-message" className="w-full max-w-md overflow-hidden rounded-[24px] border border-[#dfe5db] bg-white shadow-[0_26px_90px_rgba(0,0,0,.22)]">
