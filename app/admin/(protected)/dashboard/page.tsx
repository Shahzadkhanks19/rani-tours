...
import { Activity, ArrowUpRight, CarFront, MapPinned, PackageCheck } from "lucide-react";
...
export default async function AdminDashboardPage(){
  ...
  const [recentActivity,totalPackages,publishedPackages,totalTaxi,publishedTaxi,totalDestinations,publishedDestinations,totalFleet,publishedFleet]=await Promise.all([
  ...
  return <div className="space-y-7">
    ...
    <section className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
      <article className="rounded-2xl border border-[#143124]/10 bg-white p-5 md:p-6"><h2 className="text-xl font-black">CMS roadmap</h2><p className="mb-5 text-sm text-[#7b8d7e]">The first four reusable content modules are active.</p><div className="grid gap-3 sm:grid-cols-2">{modules.map((module,index)=><div key={module} className="flex items-center gap-3 rounded-xl border border-[#143124]/8 bg-[#f8faf6] px-4 py-3"><span className={`grid size-7 shrink-0 place-items-center rounded-full text-xs font-black ${index<4?"bg-[#b3df24] text-[#073b25]":"bg-[#e8eee8] text-[#6d806f]"}`}>{index+1}</span><div><p className="text-sm font-semibold">{module}</p><p className="text-xs text-[#8a998c]">{index<4?"Live":"Queued"}</p></div></div>)}</div></article>
