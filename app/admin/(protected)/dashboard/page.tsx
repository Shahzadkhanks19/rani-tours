...
import { Activity, ArrowUpRight, CarFront, GalleryHorizontal, MapPinned, MessageSquareText, PackageCheck } from "lucide-react";
...
export default async function AdminDashboardPage(){
  ...
  const [recentActivity,totalPackages,publishedPackages,totalTaxi,publishedTaxi,totalDestinations,publishedDestinations,totalFleet,publishedFleet,totalGallery,publishedGallery,totalEnquiries,newEnquiries]=await Promise.all([
  ...
  return <div className="space-y-7">
    <section><p className="text-sm font-semibold uppercase tracking-[0.18em] text-[#0b6b3a]">Overview</p><h1 className="mt-1 text-3xl font-black md:text-4xl">Admin Dashboard</h1><p className="mt-2 max-w-2xl text-[#6d806f]">Content management and the unified enquiry inbox share the same protected data and activity-log foundation.</p></section>
    <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">{cards.map(card=>{const Icon=card.icon;return <Link href={card.href} key={card.label} className="rounded-2xl border border-[#143124]/10 bg-white p-5 shadow-sm transition hover:-translate-y-0.5"><div className="flex items-start justify-between"><div className="grid size-11 place-items-center rounded-xl bg-[#eef5df] text-[#0b6b3a]"><Icon className="size-5"/></div><ArrowUpRight className="size-4 text-[#9aaa9b]"/></div><p className="mt-6 text-3xl font-black">{card.value}</p><p className="mt-1 font-semibold">{card.label}</p><p className="mt-1 text-sm text-[#7b8d7e]">{card.detail}</p></Link>})}</section>
    <section className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
      <article className="rounded-2xl border border-[#143124]/10 bg-white p-5 md:p-6"><h2 className="text-xl font-black">CMS roadmap</h2><p className="mb-5 text-sm text-[#7b8d7e]">Six core modules are active.</p><div className="grid gap-3 sm:grid-cols-2">{modules.map((module,index)=><div key={module} className="flex items-center gap-3 rounded-xl border border-[#143124]/8 bg-[#f8faf6] px-4 py-3"><span className={`grid size-7 shrink-0 place-items-center rounded-full text-xs font-black ${index<6?"bg-[#b3df24] text-[#073b25]":"bg-[#e8eee8] text-[#6d806f]"}`}>{index+1}</span><div><p className="text-sm font-semibold">{module}</p><p className="text-xs text-[#8a998c]">{index<6?"Live":"Queued"}</p></div></div>)}</div></article>
