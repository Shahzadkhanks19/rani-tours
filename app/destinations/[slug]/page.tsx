...
export default async function DestinationPage({params}:P){const{slug}=await params;await connectToDatabase();const d=await Destination.findOne({slug,status:"published"}).lean();if(!d)notFound();const seo=d.seo as{metaDescription?:string}|undefined;const description=cleanDescription(seo?.metaDescription||d.shortDescription);const url=absoluteUrl(`/destinations/${slug}`),image=String((d.heroImage as{url?:string}|undefined)?.url||"");const place={"@context":"https://schema.org","@type":"TouristDestination",name:String(d.title),description,url,image:image||undefined,touristType:"Travelers"};const breadcrumb={"@context":"https://schema.org","@type":"BreadcrumbList",itemListElement:[{"@type":"ListItem",position:1,name:"Home",item:absoluteUrl("/")},{"@type":"ListItem",position:2,name:"Destinations",item:absoluteUrl("/destinations")},{"@type":"ListItem",position:3,name:String(d.title),item:url}]};return <><meta name="description" content={description}/><Header/><main><script type="application/ld+json" dangerouslySetInnerHTML={{__html:jsonLd(place)}}/><script type="application/ld+json" dangerouslySetInnerHTML={{__html:jsonLd(breadcrumb)}}/><DestinationDetail destination={JSON.parse(JSON.stringify(d))}/></main><Footer/><FloatingActions/></>}
