...
function passwordVersion(value:unknown){return value instanceof Date?Math.floor(value.getTime()/1000):0}

export async function hashPassword(password:string){const salt=randomBytes(16).toString("hex");const derivedKey=(await scrypt(password,salt,64,{N:16384,r:8,p:1})) as Buffer;return `${salt}:${derivedKey.toString("hex")}`}
export async function verifyPassword(password:string,storedHash:string){const[salt,storedKeyHex]=storedHash.split(":");if(!salt||!storedKeyHex||!/^[a-f0-9]+$/i.test(storedKeyHex))return false;const storedKey=Buffer.from(storedKeyHex,"hex");const derivedKey=(await scrypt(password,salt,storedKey.length,{N:16384,r:8,p:1})) as Buffer;return storedKey.length===derivedKey.length&&timingSafeEqual(storedKey,derivedKey)}
...
export function validatePasswordStrength(password:string){return password.length>=12&&password.length<=128&&/[a-z]/.test(password)&&/[A-Z]/.test(password)&&/\d/.test(password)&&/[^A-Za-z0-9]/.test(password)&&!/(password|qwerty|admin|rani tours|123456)/i.test(password)}
export async function logAdminActivity(input:{adminId?:string|null;adminName?:string;action:string;entity:string;entityId?:string|null;metadata?:Record<string,unknown>}){await connectToDatabase();const headerStore=await headers();const ipAddress=headerStore.get("x-forwarded-for")?.split(",")[0]?.trim()||headerStore.get("x-real-ip");await ActivityLog.create({adminId:input.adminId||null,adminName:input.adminName||"System",action:input.action.slice(0,120),entity:input.entity.slice(0,80),entityId:input.entityId||null,metadata:input.metadata||{},ipAddress:ipAddress?.slice(0,80)||null})}
export function createResetToken(){const raw=randomBytes(32).toString("hex");const hash=createHmac("sha256",getSessionSecret()).update(raw).digest("hex");return{raw,hash}}
export function hashResetToken(raw:string){return createHmac("sha256",getSessionSecret()).update(raw).digest("hex")}
